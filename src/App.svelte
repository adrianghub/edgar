<script lang="ts">
  ...
  import type {IMessage, IState} from "./lib/store/main.dt";
  import {defaultSystemPrompt, loadState, saveState, state} from "./lib/store/main";
...
<main>
  ...
  <div class="text-center text-zinc-200 p-8">
    <div class="flex flex-col">
      <div class="text-zinc-200 flex text-lg font-bold justify-end">
        Assistant
      </div>
      ...
      <div class="text-left mt-3.5 overflow-y-auto output p-2.5">
        {#each currentState.messages as message}
          <div class="mb-2.5">
            ...
            {:else}
              <div class="font-bold opacity-50">{message.role === 'assistant' ? 'Edgar' : 'You'}:</div> {@html marked(message.content)}
            {/if}
          </div>
        {/each}
      </div>
